...
export class DocTree {
    ...
    async search(query: string) {
        ...
        const data = await Promise.all(
            Object.entries(this._readPromises)
                .map(async([file, prom]) => {
    ...
    // the _renderCache
    async renderMarkdown(file: string, useCache: boolean = true): Promise<RenderResult | false> {
        const f = await this._sanitizeFilePath(file);
        if (!f) return false;
    ...
    private _buildTree() {
        const parse = (c: any) => {
            c.filePath = c.path;
            c.path = `${this.prefix}/${path.relative(this.directory, c.path)}`.replace('.md', '');
        ...
        const tree = dt(
            this.directory,
            {
                extensions: /\.md/, exclude: /\/index\.md/
            },
            (item: any) => {
